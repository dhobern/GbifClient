...
public class OccurrenceMatrix<S extends CellValue & Comparable,T extends Mappable> {
    ...
    private CellValueFactory factory;
    ...
    public S insert(T m) {
        ...
        if (cellValue == null) {
            cellValue = (S) factory.createCellValue(position, m);
    ...
    public void exportGrid(String fileName) {
        if (!dimensions.isAbstractMatrix()) {
            try {
                ...
                String formatString = "\titem_%0" + digits + "d";
                for (int i = 0; i < maxCellSize; i++) {
                   writer.printf(formatString, i + 1);
                }
                writer.printf("\n");
